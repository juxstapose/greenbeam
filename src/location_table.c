...
void Location_Update_By_Userkey(sqlite3* db, Location* location, int user_key, LogConfig* log_config) {
	char buffer[SQL_STRING_MAX_SIZE] = {'\0'};
	int bytes = sprintf(buffer, "UPDATE location SET location_user_key = '%i', location_zonename = '%s', location_x = '%i', location_y = '%i' WHERE location_user_key = '%i';", user_key, location->zonename, location->x, location->y, user_key);
...
Location* Location_Find_By_Userkey(sqlite3* db, int user_key, LogConfig* log_config) {
	...
	if(rc != SQLITE_OK) {
		...
		sqlite3_bind_int(result, 1, user_key);	
	}

	rc = sqlite3_step(result);

	if(rc == SQLITE_ROW) {
		int r_locationkey = sqlite3_column_int(result, 0);
		...
		Location* location = Location_Create(r_location_key, r_userkey, r_zonename, r_location_x, r_location_y);
		...
		Log_log(log_config, LOG_ERROR, "failed to fetch data with user key: %s\n", user_key);
		return NULL;
	} else {
